...
export default function LandingContent() {
  ...
  const handleInstall = async () => {
    ...
    if (choice) console.info('[pwa] userChoice:', choice)
  }
        ...
        <div className="relative max-w-md mx-auto px-6 pt-16 pb-12">
          ...
          {canInstall && !isStandalone ? (
            <button
              ...
              className="w-full h-14 flex items-center justify-center gap-2 py-4 bg-transparent border border-[#00A651] text-[#00A651] rounded-2xl text-[16px] font-bold mb-4 transition-colors hover:bg-[#00A651]/20 active:bg-[#00A651]/30"
            >
              앱 설치하기 <Download size={18} />
            </button>
          ) : isIOS && !isStandalone ? (
            <p className="text-center text-[12px] text-white/70 mb-4">
              공유 버튼 → &quot;홈 화면에 추가&quot;를 선택해 주세요.
            </p>
          ) : null}
        ...
        <div className="max-w-md mx-auto text-center">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              onClick={handleGoogleSignIn}
              onMouseEnter={() => router.prefetch('/trainer/dashboard')}
              className="flex items-center justify-center gap-1.5 py-3 bg-white rounded-xl text-[12px] font-semibold text-[#1A1A1A]"
            >
