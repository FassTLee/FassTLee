...
export default function LessonClient({ chapterId, chapterTitle, subjectName, questions }: Props) {
  ...
  useEffect(() => {
    ...
    const isM = (s ?? 'conceptualizer') === 'memorizer'
    const maxSlides = isM ? 3 : 5
    setSlides(questions.slice(0, maxSlides))
    setMounted(true)
  }, [questions])
  ...
  useEffect(() => {
    ...
    if (slideMode !== 'auto' || showMiniQuiz || !mounted) return
  ...
  }, [slideMode, slideIndex, showMiniQuiz, mounted])
  ...
  const onDragStart = (clientX: number) => { dragStartX.current = clientX; isDragging.current = true }
    ...
    <div className="min-h-screen bg-[#F5F5F3] flex flex-col">
      ...
      >
        ...
        {slides.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <p className="text-[14px] text-[#ADADAD]">학습 콘텐츠가 없습니다.</p>
          </div>
        ) : (
          <>
      ...
      {slideMode === 'auto' && !showMiniQuiz && slides.length > 0 && (
        <div className="h-1 bg-[#E5E5E5] flex-shrink-0">
          <div className="h-full bg-[#E24B4A] transition-none" style={{ width: `${autoProgress}%` }} />
        </div>
      )}
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-4 px-4 py-4 rounded-2xl border-2 text-left transition-all ${
                      miniConfirmed
                        ? isCorrect ? 'border-[#639922] bg-[#63992210]'
                          : isWrong ? 'border-[#E24B4A] bg-[#E24B4A10]'
                          : 'border-[#E5E5E5] bg-[#F5F5F3] opacity-50'
                        : miniSelected === idx ? 'border-[#E24B4A] bg-[#E24B4A]/5' : 'border-[#E5E5E5]'
                    }`}
                  >
