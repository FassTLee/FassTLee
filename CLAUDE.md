...
- 배포 전 확인 순서:
  ...
  4. `git diff --stat`으로 변경 파일 전수 보고
     파일 반영 확인의 근거는 도구가 수행한 파일 재판독 결과입니다.
     `git diff`는 의도한 변경만 보여주므로 단독 근거가 되지 못합니다.
     채팅에 옮겨 붙인 파일 내용도 원본과 다를 수 있으므로 단독 근거가 아닙니다.
     재판독 보고에는 대상 문자열의 등장 횟수와 행 번호를 포함시킵니다.
